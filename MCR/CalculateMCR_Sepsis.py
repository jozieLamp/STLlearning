...
def runMCRGenRules(patient):
    ...
    # Load all rules
    filepath = "Data/Sepsis/Rules/" + str(patient) + "ruleScores.txt"
    posRules, negRules = readRulesFromFile(filepath)

    # Load Data
    data = pd.read_csv('Data/Sepsis/DataFrames/' + str(patient) + 'DataFrame.csv', index_col=0)
    labels = pd.read_csv('Data/Sepsis/DataFrames/' + str(patient) + 'Labels.csv')
    ...
    # print("\nNegative rules")
    # print(negDF)
    posDF.to_csv("Data/Sepsis/MCR/" + str(patient) + 'PositiveMCR.csv', index=False)
    negDF.to_csv("Data/Sepsis/MCR/" + str(patient) + 'NegativeMCR.csv', index=False)
    ...
    giantDF = posDF.append(negDF)
    giantDF = giantDF.sort_values(by='Accuracy', ascending=False)
    giantDF.to_csv("Data/Sepsis/MCR/" + str(patient) + 'AllMCR.csv', index=False)
    ...
    with open("Data/Sepsis/Best/" + repr(patient) + "Rules.txt", 'w') as filehandle:
        for b in bestRules:
