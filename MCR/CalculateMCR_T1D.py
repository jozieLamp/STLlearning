...
def runMCRGenRules(patient):
    ...
    # Load all rules
    filepath = "Data/T1D/Rules/" + str(patient) + "ruleScores.txt"
    posRules, negRules = readRulesFromFile(filepath)

    # Load Data
    data = pd.read_csv('Data/T1D/DataFrames/' + str(patient) + 'DataFrame.csv', index_col=0)
    labels = pd.read_csv('Data/T1D/DataFrames/' + str(patient) + 'Labels.csv')
    ...
    # print("\nNegative rules")
    # print(negDF)
    posDF.to_csv("Data/T1D/MCR/" + str(patient) + 'PositiveMCR.csv', index=False)
    negDF.to_csv("Data/T1D/MCR/" + str(patient) + 'NegativeMCR.csv', index=False)
    ...
    giantDF.to_csv("Data/T1D/MCR/" + str(patient) + 'AllMCR.csv', index=False)
    ...
    with open("Data/T1D/Best/" + repr(patient) + "Rules.txt", 'w') as filehandle:
        for b in bestRules:
